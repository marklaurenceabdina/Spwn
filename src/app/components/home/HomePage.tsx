...
import { Play, ChevronRight, Heart, TrendingUp, Trophy, Zap } from "lucide-react";
...
export function HomePage() {
  ...
  const SPEED = 40; // px per second (smooth)
  ...
  return (
    <div className="flex flex-col" style={{ background: "var(--spwn-bg)" }}>
      ...
      >
        {/* Featured selector moved to admin panel */}
      ...
      {featuredGames.length > 1 && (
        <div className="mt-6 px-4">
          <SectionHeader icon={Zap} title="Featured Games" />
            ...
            <div className="flex gap-3" style={{ width: "max-content" }}>
              {featuredGames.slice(1).map((game) => (
                <div
                  ...
                  className="rounded-xl overflow-hidden shrink-0 cursor-pointer p-3 active:scale-[0.98] transition-all"
                  style={{ background: "var(--spwn-card)", border: "1px solid var(--spwn-border)", width: 160 }}
                >
                  <img src={game.image} alt={game.title} className="w-full h-24 rounded-lg object-cover mb-2" />
                  <p className="text-white text-xs truncate" style={{ fontWeight: 600 }}>{game.title}</p>
                  <p className="text-white/40 text-xs mt-0.5 truncate">{game.year} • {game.genres[0]}</p>
                  <div className="flex items-center gap-2 mt-2">
                    <RatingStars rating={game.rating} className="px-1.5 py-0.5 rounded text-xs" />
                    <button onClick={(e) => { e.stopPropagation(); toggleWishlist(e, game.id) }} className="ml-auto">
                      <Heart size={12} fill={getBacklogStatus(game.id) === "want" ? "var(--spwn-accent)" : "none"} stroke={getBacklogStatus(game.id) === "want" ? "var(--spwn-accent)" : "rgba(255,255,255,0.3)"} />
                    </button>
                  </div>
                </div>
              ))}
      ...
      <div className="mt-6 px-4">
        ...
        <div className="overflow-hidden relative">
          ...
          >
            {loopTopRated.map((game, i) => {
              const rank = (i % topRated.length) + 1;
              ...
              return (
                <div
                  ...
                  style={{ width: 140, background: "var(--spwn-card)", border: "1px solid var(--spwn-border)", scrollSnapAlign: "start" }}
                >
                  ...
                  <div className="absolute top-2 left-2 w-5 h-5 rounded-full flex items-center justify-center overlay-white"
                    style={{ background: medalColors[rank - 1] || medalColors[0], fontSize: 10, fontWeight: 800 }}>
                    {rank}
                  </div>
